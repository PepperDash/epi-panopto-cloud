    ...
    public abstract partial class ABaseDriverProtocol
    {
        ...
        public virtual void Initialize(object driverData)
        {
            if (driverData != null && driverData.GetType() == typeof(BaseRootObject))
            {
                ...
                try
                {
                    ...
                    if (BaseDeviceProtocolInitialized.Exists())
                    {
                        BaseDeviceProtocolInitialized();
                    }
                }
                catch (Exception e)
                {
        ...
        public virtual void Tick(object obj)
        {
            ...
            if (TickCounter == 0 && PollingEnabled)
            {
                ...
                {
                    Log("Polling has failed for some reason. Please check error logs for more detail.");
                    throw;
                }
            }
        ...
        protected void RemoveCommandFromQueue()
        {
            ...
            if (command != null)
            {
                ...
                else if ((WarmingUp || CoolingDown) && command.CommandGroup == CommonCommandGroupType.Power && command.IsPollingCommand)
                {
                    SendToTransport(command);
                }
                else
